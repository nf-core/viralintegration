...
virus_count_info = virus_count_info %>% mutate(frac_covered = n_bases_covered / seqlen) %>% filter(frac_covered > 1e-3)
virus_count_info = virus_count_info %>% arrange(desc(frac_covered)) %>% mutate(frac_covered = sprintf("%.3f", frac_covered))
